 ...
 *
 * 变更这里之前请先读本仓库文档中「前后端同源化」一节。
 */

/** 后端公开域名。改这里等于切换本前端对应的后端。 */
const API_ORIGIN = "https://ccbserver.baka.website";
...
export function middleware(context) {
  ...
  // 只接管 /api/*，其余（首页、静态资源、sitemap、robots）原样放行。
  // 注意：本仓库 netlify.toml 里有一条 `/* -> /index.html` 的 SPA fallback，
  // 必须确保 /api/* 先落到这里而不是被 fallback 吞掉。
